...
public:
    ...
    void * alloc(size_t size, size_t alignment = 0)
    {
        ...
        if (size >= mmap_threshold)
        {
            if (alignment > MMAP_MIN_ALIGNMENT)
                throw DB::Exception("Too large alignment " + formatReadableSizeWithBinarySuffix(alignment)
            ...
            if (MAP_FAILED == buf)
                DB::throwFromErrno(
                    "Allocator: Cannot mmap " + formatReadableSizeWithBinarySuffix(size) + ".", DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY);
        ...
        else
        {
            if (alignment <= MALLOC_MIN_ALIGNMENT)
            {
                ...
                if (nullptr == buf)
                    DB::throwFromErrno("Allocator: Cannot malloc " + formatReadableSizeWithBinarySuffix(size) + ".",
            ...
            else
            {
                ...
                if (0 != res)
                    DB::throwFromErrno("Cannot allocate memory (posix_memalign) " + formatReadableSizeWithBinarySuffix(size) + ".",
    ...
    void free(void * buf, size_t size)
    {
        if (size >= mmap_threshold)
        {
            if (0 != munmap(buf, size))
                DB::throwFromErrno(
                    "Allocator: Cannot munmap " + formatReadableSizeWithBinarySuffix(size) + ".", DB::ErrorCodes::CANNOT_MUNMAP);
        }
        else
        {
            ::free(buf);
    ...
    void * realloc(void * buf, size_t old_size, size_t new_size, size_t alignment = 0)
    {
        ...
        else if (old_size < mmap_threshold && new_size < mmap_threshold && alignment <= MALLOC_MIN_ALIGNMENT)
        {
            ...
            if (nullptr == new_buf)
                DB::throwFromErrno("Allocator: Cannot realloc from " + formatReadableSizeWithBinarySuffix(old_size) + " to "
        ...
        else if (old_size >= mmap_threshold && new_size >= mmap_threshold)
        {
            ...
            if (MAP_FAILED == buf)
                DB::throwFromErrno("Allocator: Cannot mremap memory chunk from " + formatReadableSizeWithBinarySuffix(old_size) + " to "
                        + formatReadableSizeWithBinarySuffix(new_size) + ".",
