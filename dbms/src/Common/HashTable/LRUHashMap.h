...
struct LRUHashMapCellNodeTraits
{
    ...
    static node * getNext(const node * ptr) { return ptr->next; }
    static void setNext(node * __restrict ptr, node * __restrict next) { ptr->next = next; }
    static node * getPrevious(const node * ptr) { return ptr->prev; }
    static void setPrevious(node * __restrict ptr, node * __restrict prev) { ptr->prev = prev; }
};
