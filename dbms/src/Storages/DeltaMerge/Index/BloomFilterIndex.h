...
public:
    ...
    {}
    explicit BloomFilterIndex(
        std::vector<BloomFilterPtr> & bloom_filter_vec_,
        double false_positive_probability_ = 0.01)
    ...
    RSResults checkEqual(size_t start_pack, size_t pack_count, const Field & value, const DataTypePtr & type) const;
...
private:
    static void updateBloomFilter(
        BloomFilterPtr & bloom_filter,
        const IColumn & column,
        size_t size,
        const IDataType * type);
    // 一个 dmfile 的一个 column 对应一个 bloom_filter_vec（目前先是以前支持 index 的现在也支持），vec 长度等于 pack numbers，也就是一个 pack 一个
