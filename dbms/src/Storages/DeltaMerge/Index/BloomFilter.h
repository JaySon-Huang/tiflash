...
public:
    ...
    bool equal(const BloomFilter & f)
    {
        if ((salt_count_ == f.salt_count_) && (table_size_ == f.table_size_) && (random_seed_ == f.random_seed_)
            && (projected_element_count_ == f.projected_element_count_)
            && (inserted_element_count_ == f.inserted_element_count_)
            && (desired_false_positive_probability_ == f.desired_false_positive_probability_)
            && (bit_table_.size() == f.bit_table_.size()) && (salt_.size() == f.salt_.size()))
        {
            for (std::size_t i = 0; i < bit_table_.size(); ++i)
            {
                if (bit_table_[i] != f.bit_table_[i])
                {
                    return false;
                }
            }
    ...
    void write(DB::WriteBuffer & buf)
    {
        DB::writeIntBinary(salt_count_, buf);
        DB::writeIntBinary(table_size_, buf);
        DB::writeIntBinary(projected_element_count_, buf);
        DB::writeIntBinary(inserted_element_count_, buf);
        DB::writeIntBinary(random_seed_, buf);
        DB::writeFloatBinary(desired_false_positive_probability_, buf);
        DB::writeIntBinary(bit_table_.size(), buf);
        buf.write(reinterpret_cast<const char *>(bit_table_.data()), bit_table_.size());
        DB::writeIntBinary(salt_.size(), buf);
        for (auto & salt : salt_)
        {
            DB::writeIntBinary(salt, buf);
        }
    }

    static std::shared_ptr<BloomFilter> read(DB::ReadBuffer & buf)
    {
        std::shared_ptr<BloomFilter> bloom_filter = std::make_shared<BloomFilter>();
        DB::readIntBinary(bloom_filter->salt_count_, buf);
        DB::readIntBinary(bloom_filter->table_size_, buf);
        DB::readIntBinary(bloom_filter->projected_element_count_, buf);
        DB::readIntBinary(bloom_filter->inserted_element_count_, buf);
        DB::readIntBinary(bloom_filter->random_seed_, buf);
        DB::readFloatBinary(bloom_filter->desired_false_positive_probability_, buf);
        size_t bit_table_size = 0;
        DB::readIntBinary(bit_table_size, buf);
        bloom_filter->bit_table_.resize(bit_table_size);
        buf.read(reinterpret_cast<char *>(bloom_filter->bit_table_.data()), bit_table_size);
        size_t salt_size = 0;
        DB::readIntBinary(salt_size, buf);
        bloom_filter->salt_.resize(salt_size);
        for (size_t index = 0; index < salt_size; index++)
        {
            DB::readIntBinary(bloom_filter->salt_[index], buf);
        }
        return bloom_filter;
    }
