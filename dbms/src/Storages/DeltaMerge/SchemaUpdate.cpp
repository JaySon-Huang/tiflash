...
{
    auto cast_default_value = [&](const Field & value, const DataTypePtr & type) {
        auto cast_default_value_impl = [&](auto self, const Field & value, const DataTypePtr & type) -> Field {
            ...
            case TypeIndex::Nullable:
            {
                ...
                return self(self, value, nested_type); // Recursive call on nested type
            }
            default:
        ...
        return cast_default_value_impl(cast_default_value_impl, value, type);
    };
    ...
    if (command.default_expression)
    {
        ...
        {
            throw DB::Exception{
                fmt::format("(in setColumnDefineDefaultValue for default_expression: {})",
