...
{
extern const Metric DT_NumStorageDeltaMerges;
} // namespace CurrentMetrics
...
StorageDeltaMerge::StorageDeltaMerge(
    ...
    , holder_counter(CurrentMetrics::DT_NumStorageDeltaMerges, 1)
    , store_inited(false)
    ...
    const Settings & settings)
{
    return getAndMaybeInitStore()
        ->buildSegmentsFromCheckpointInfo(global_context, cancel_handle, settings, range, checkpoint_info);
}
