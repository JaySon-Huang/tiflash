...
PageId
PageStorage::getNormalPageId(PageId page_id, SnapshotPtr snapshot)
{
    if (!snapshot)
    {
...
std::tuple<bool, UInt64> PageStorage::getAppliedVersion(PageId page_id)
{
    ...
    { // this may throw an exception if ref to non-exist page
        if (const auto entry = toConcreteSnapshot(snapshot)->version()->find(page_id); entry)
...
bool PageStorage::gc(bool not_skip, const WriteLimiterPtr & write_limiter, const ReadLimiterPtr & read_limiter)
{
    ...
    PageEntriesEdit gc_file_entries_edit;
    {
        ...
        DataCompactor<VersionedPageEntries::SnapshotPtr> compactor(*this, gc_context.calculateGcConfig(config), write_limiter, read_limiter);
