...
/// Read an unsigned integer in plain format from a non-0-terminated string.
...
/// Read a signed integer in plain format from a non-0-terminated string.
...
/// Read a floating-point number in plain format, with rough rounding, from a non-0-terminated string.
...
JSON::ElementType JSON::getType() const
{
    ...
    case '"':
    {
        /// Let's check if this is a simple string or a name-value pair.
...
JSON::Pos JSON::skipString() const
{
    ...
    /// Fast path: find the next double quote. If there is no backslash before it, then it signifies the end of the string (assuming the JSON is valid).
    Pos closing_quote
    ...
    do
    {
        ...
        case '\\':
            ...
            case 'u':
            {
                ...
                buf.resize(buf.size() + 6); /// The maximum size of a UTF-8 multibyte sequence.
